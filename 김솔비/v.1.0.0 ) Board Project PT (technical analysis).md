...
지난 프로젝트를 다시 뜯어보고 파악하는 시간을 가져보려 한다. 😊


본론에 앞서 개발 스택과 자세히 알아볼 주요 키워드 목록과 파일구조를 아래에서 확인 가능하다.

2023-11-28 간단하게 오늘은 JPA 데이터 흐름도 즉, 데이터의 요청과 응답에 대해 이야기 하겠다. 
...
2. **레파지토리 계층 (JPA 사용)**: JPA를 통해 데이터베이스로부터 데이터를 검색하고, 이를 Entity 객체로 변환한다.
